...
<div class="container workspace">
 ...
 -->
    ...
    <div class="page-inner" id="page-new-project" data-page-name="创建新项目" >
		...
		<form action="api/editeAPI" method="post" onsubmit="return checkForm(this);">
			...
			<table cellpadding="0" border="1" cellpadding="0" cellspacing="0"
				...
				<tr>
					...
					<td>
						<input type="text" style="display: inline-block; width: 50%;" id="url" name="url" value="${one.url }" size="114" /> <a href="javaScript:void(0);" onclick="parseURL();">自动产生参数和结果</a>(需开启此URL相应服务)
					</td>
				</tr>
				...
				<tr >
					...
					<td>
						<input type="text" id="name" name="name" value="${one.name }" size="114" maxlength="20"/>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						<textarea id="des" name="des" style='height: 50px;width: 80%;'>${one.des }</textarea>
					</td>
				</tr>
				<tr>
					<td align="left">
				...
				<tr>
					...
					<td style='text-align: left;'>
						<select id="needCookie" name="needCookie">
							<option value="是" ${one.needCookie eq "是" ? "selected" : "" } ${needCookie eq "是" ? "selected" : "" }>
								是
							</option>
							<option value="否" ${one.needCookie eq "否" ? "selected" : "" } ${needCookie eq "否" ? "selected" : "" }>
								否
							</option>
						</select>
					</td>
				...
				<tr>
					...
					<td style='text-align: left;'>
						...
						<input value="" name="versionInput" id="versionInput" <c:if test="${not empty confInf.versionList }">style="display:none"</c:if>/>
				...
				<tr>
					...
					<td>
						<table id="para_table" cellpadding="0" border="1" cellpadding="0" cellspacing="0"
							...
							<c:if test="${not empty para_name && 1==5 }">
								<%
									...
									for(int i =0 ; i< para_name.length ; i++){
										%>
										<tr>
											<td><input name='para_name' value='<%=para_name[i] %>' /></td>
											<td><select name='para_type'><option value='String' <%= "String".equals(para_type[i]) ? "selected" :""  %>>String</option><option value='JSONArray' <%= "JSONArray".equals(para_type[i]) ? "selected" :""  %>>JSONArray</option><option value='JSONObject' <%= "JSONObject".equals(para_type[i]) ? "selected" :""  %>>JSONObject</option><option value='Number' <%= "Number".equals(para_type[i]) ? "selected" :""  %>>Number</option></select></td>
											<td><select name='para_isNeed'><option value='是' <%= "是".equals(para_isNeed[i]) ? "selected" :""  %>>是</option><option value='否' <%= "否".equals(para_isNeed[i]) ? "selected" :""  %>>否</option></select></td>
											<td><input  name='para_des' value='<%=para_des[i] %>' /></td>
											<td><input  name='para_caseVal' value='<%=para_caseVal[i] %>' /></td>
							...
							<c:forEach var="obj" items="${one.parameter }">
								<tr>
									<td><input style='width: 80px;' name='para_name' value='${obj.para_name }' /></td>
									<td><select style='width: 120px;' name='para_type'><option value='String' ${obj.para_type eq "String"  ? "selected" :"" }>String</option><option value='JSONArray' ${obj.para_type eq "JSONArray" ? "selected" :"" }>JSONArray</option><option value='Number' ${obj.para_type eq "Number" ? "selected" :"" }>Number</option></select></td>
									<td><select style='width: 40px;' name='para_isNeed'><option value='是' ${obj.para_isNeed eq "是" ? "selected" :"" } >是</option><option value='否' ${obj.para_isNeed eq "否" ? "selected" :"" }>否</option></select></td>
									<td><input style='width: 80px;'  name='para_des' value='${obj.para_des }' /></td>
									<td><input style='width: 80px;'  name='para_caseVal' value='${obj.para_caseVal }' /></td>
									<td><a href='javaScript:void(0);' onclick='removeRow(this.parentNode.parentNode);'>删除</a></td>
								</tr>
							</c:forEach>
						</table>
				...
				<tr>
					...
					<td>
						<textarea id="result" name="result" style='height: 100px;width: 80%;'>${one.result }</textarea>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						<textarea id="other" name="other" style='height: 80px;width: 80%;'>${one.other }</textarea>
					</td>
				</tr>
				...
				<tr align="center">
					<td colspan="2">
						<input type="submit" class="btn" value="修改并保存" />
					</td>
				</tr>
			</table>
		</form>
