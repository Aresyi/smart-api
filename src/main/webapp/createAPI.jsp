								...
								<tr >
									...
									<td style='text-align: left;'>
										<c:forEach var="one" items="${obj.value}" varStatus="vv">
											<input class="${obj.key}" name="noticEmail" style='vertical-align: middle' type="checkbox" value = "${one.email }"><span>&nbsp;${one.name }</span> <c:if test="${ not empty one.openId }"><i class="fa fa-weixin"></i></c:if>&nbsp;&nbsp;
										</c:forEach>
									</td>
								</tr>
							</c:forEach>
